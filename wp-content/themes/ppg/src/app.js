window.addEventListener('DOMContentLoaded', (ev) => {
	...
	$('.um-profile-edit').insertAfter( $('.um-field-image') );
	...
	$('.woof_shortcode_output').each(function(){
		var $product_length = $(this).find('.products').children().length;
		// console.log($product_length > 0);
		if ( $product_length > 0 ) {
			// Hide Empty Image
	...
	$('.um.um-login').each(function(){
		...
		var $thisPwd = $(this).find('.um-field.um-field-password');
		// console.log($thisSubmitBlock);
	...
	$('.profile__contact-button .um-members-messaging-btn .um-message-btn').text('Message This Breeder');

	// Breeder Search
	var $searchFilterSidebar = $('#search-filters');
	var $searchFilters = $searchFilterSidebar.find('.um-search-filter');
		...
		$(window).scroll(function(){
			$('.animals__bg-top').each(function(){
				if ($(this).offset().top < $(window).scrollTop()) {
					...
					var half = (difference / 2) + 'px', transform = 'translate3d( 0, ' + half + ',0)';
	...
	$pop.each(function(){
		...
		$btn.on('click', function(){
			...
			Cookies.set('list-pet-close', true, { expires: 1, path: '' }); 
		});
		$this.on('click', function(){
			$this.removeClass('footer-list-pet--active');
			Cookies.set('list-pet-close', true, { expires: 1, path: '' }); 
	...
	// Image Previews
	var $add_listing = $(".pet-listing__add-listing");
	// console.log($add_listing);
