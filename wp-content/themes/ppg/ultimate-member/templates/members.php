	...
	<?php if ( $search && $show_search ) { ?>
		<div class="um-member-directory-header-row um-member-directory-search-row">
			<div class="um-member-directory-search-line">
				<div>
					<label class="screen-reader-text" for="search"><?php _e( 'Search:', 'ultimate-member' ); ?></label>
					<input type="search" class="um-search-line" placeholder="<?php esc_attr_e( 'Search for breeders, bloodline, location, etc.', 'ultimate-member' ) ?>"  value="<?php echo esc_attr( $search_from_url ) ?>" aria-label="<?php esc_attr_e( 'Search', 'ultimate-member' ) ?>" speech />
				</div>
				<button class="um-do-search">
					<?= file_get_contents(get_template_directory() . '/img/icons/search.svg'); ?>
				</button>
			</div>
		</div>
	<?php } ?>

	<div class="um-member-directory-header-row um-member-directory-details-row">
		...
		<?php /* if ( $search && $show_search ) { ?>
			<div class="um-member-directory-header-row um-member-directory-search-row">
				<div class="um-member-directory-search-line">
					...
					<input type="button" class="um-do-search um-button" value="<?php esc_attr_e( 'Search for breeders, bloodline, location, etc.', 'ultimate-member' ); ?>" />
				</div>
			</div>
		<?php } 
		...
		<?php if ( $filters && $show_filters && count( $search_filters ) ) {
			...
			if ( is_array( $search_filters ) ) { ?>
				<script type="text/template" id="tmpl-um-members-filtered-line">
					<#
						var filters = document.getElementsByClassName("um-search-filter");
						var clearFilters = document.getElementsByClassName("um-clear-filters-a");
						var removeFilter = document.querySelectorAll("um-members-filter-remove");

						clearFilters[0].addEventListener('click', function (event) {
							if (!event.target.matches('.um-clear-filters-a')) return;
							event.preventDefault();
							filters.forEach(filter => {
								if( filter.children[0].id == 'Dogs' || filter.children[0].id == 'Cats' || filter.children[0].id == 'Birds' || filter.children[0].id == 'Farm Life' || filter.children[0].id == 'Horses' || filter.children[0].id == 'Reptiles' || filter.children[0].id == 'Small Pets' ) {
						...
						filters.forEach(filter => {
							...
							data.filters.forEach( currentFilter => {
								if(filter.children[0].id == currentFilter.value) {
						...
						<# _.each( data.filters, function( filter, key, list ) { #>
							<div class="um-members-filter-tag">
								<# if ( filter.type == 'slider' ) { #>
									{{{filter.value_label}}}
								<# } else { #>
									<div><strong>{{{filter.label}}}:</strong> <span>{{{filter.value_label}}}</span></div>
								<# } #>
				...
				<div class="um-member-directory-header-row">
					<div class="um-filtered-line">
						<div class="um-clear-filters"><a href="javascript:void(0);" class="um-clear-filters-a" title="<?php esc_attr_e( 'Remove all filters', 'ultimate-member' ) ?>"><?php _e( 'Reset Filters', 'ultimate-member' ); ?></a></div>
					</div>
				</div>

				<?php //var_dump( $search_filters ); ?>

				<div id="search-filters" class="um-member-directory-header-row um-member-directory-filters-bar<?php if ( ! $filters_expanded ) { ?> um-header-row-invisible<?php } ?>">
